...
export default function LoginForm() {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 mb-4">
        <div>
          ...
          <input
            ...
            className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-indigo-500 outline-none transition-colors font-mono"
            placeholder="예: michael26 또는 teacher_kim"
            value={id}
      ...
      {/* 오류 메시지 */}
      {err && <p className="text-red-500 text-xs mb-3 text-center font-semibold">{err}</p>}

      {/* 로그인 버튼 */}
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl text-sm transition-all mb-4 shadow-md shadow-indigo-100 disabled:opacity-60"
      >
        {loading ? '로그인 중...' : '로그인'}
      </button>
      ...
      {/* 관리자 / Google 로그인 보조 링크 */}
      <div className="text-center">
        <button
          type="button"
          onClick={handleGoogle}
          disabled={loading}
          className="text-xs text-gray-400 hover:text-indigo-600 font-medium transition-colors inline-flex items-center gap-1.5"
        >
