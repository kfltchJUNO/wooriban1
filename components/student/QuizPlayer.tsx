...
export default function QuizPlayer({ quiz, onClose, onComplete }: Props) {
  ...
  const [checked, setChecked]       = useState(false)  // 현재 문항 답 확인 여부

  const currentQ  = quiz.questions[currentIdx]
  const total     = quiz.questions.length
  const myAnswer  = answers[currentQ?.id] ?? ''
  const isCorrect = myAnswer.trim().toLowerCase() === currentQ?.answer.trim().toLowerCase()
  ...
  const submitQuiz = async () => {
    ...
    const correctCount = quiz.questions.reduce((acc, q) => {
      ...
      return a.trim().toLowerCase() === q.answer.trim().toLowerCase() ? acc + 1 : acc
    }, 0)
  ...
  const TYPE_LABEL: Record<string, string> = {
    fill_blank: '빈칸 채우기', grammar: '문법 활용', idiom: '관용어', ox: 'O/X', matching: '매칭'
  }

  if (showResult) return <QuizResult quiz={quiz} answers={answers} score={score} total={total} onClose={onClose}/>
      ...
      <div className="bg-white rounded-3xl w-full max-w-[560px] max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-bold text-gray-400">{quiz.title}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-1">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">{TYPE_LABEL[currentQ.type] ?? currentQ.type}</span>
          </div>
          ...
          {currentQ.type === 'ox' && (
            <div className="grid grid-cols-2 gap-4">
              {['O', 'X'].map(opt => (
                <button key={opt} onClick={() => handleAnswer(opt)} disabled={checked}
                  className={`py-6 text-3xl font-black rounded-2xl border-2 transition-all
                    ${myAnswer === opt
                      ? checked
                        ? isCorrect ? 'border-green-400 bg-green-50 text-green-600' : 'border-red-400 bg-red-50 text-red-600'
          ...
          {/* 주관식 */}
          {(currentQ.type === 'fill_blank' || currentQ.type === 'grammar') && (
            <div>
              <input
              ...
              />
              {/* 힌트 (복습용) */}
              {quiz.purpose === 'review' && (currentQ as any).hint && !checked && (
                <details className="mt-2">
                  <summary className="text-xs text-indigo-600 cursor-pointer select-none font-bold">💡 힌트 보기</summary>
                  <p className="text-xs text-gray-600 mt-1 pl-2">{(currentQ as any).hint}</p>
                </details>
              )}
            </div>
          )}
          ...
          {/* 피드백 */}
          {checked && (
            <div className={`mt-4 p-4 rounded-2xl border ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className={`font-bold text-base mb-1 ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>
                {isCorrect ? '✅ 정답!' : `❌ 오답 — 정답: ${currentQ.answer}`}
              </div>
        ...
        <div className="p-5 border-t border-gray-100">
          {!checked ? (
            <button onClick={handleCheck} disabled={!myAnswer}
...
function QuizResult({ quiz, answers, score, total, onClose }: {
  quiz: Quiz; answers: Record<string, string>; score: number; total: number; onClose: () => void
}) {
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(30,27,75,0.45)] backdrop-blur-sm z-50 flex items-center justify-center p-5">
      <div className="bg-white rounded-3xl p-10 w-full max-w-[420px] text-center shadow-2xl">
        ...
        <div className="font-['Syne'] font-black text-6xl text-indigo-600 leading-none">{score}</div>
        ...
        {score < total && (
          <div className="bg-gray-50 rounded-2xl p-4 mb-6 text-left max-h-[200px] overflow-y-auto">
            ...
            {quiz.questions
              .filter(q => (answers[q.id] ?? '').trim().toLowerCase() !== q.answer.trim().toLowerCase())
              .map(q => (
                <div key={q.id} className="mb-2 pb-2 border-b border-gray-100 last:border-0">
                  <p className="text-xs text-gray-600 line-clamp-1">{q.question}</p>
                  <p className="text-xs font-bold text-green-600 mt-0.5">정답: {q.answer}</p>
                </div>
              ))}
          </div>
        )}
