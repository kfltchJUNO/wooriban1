...
export default function TextbookList({ onRefresh, onReUpload }: Props) {
  ...
  if (loading) return (
    <div className="text-center text-gray-400 py-10 text-sm animate-pulse">교재 목록 불러오는 중...</div>
  )
  if (textbooks.length === 0) return (
    <div className="text-center text-gray-400 py-10 text-sm">등록된 교재가 없어요. 교재를 업로드해주세요.</div>
  )
        ...
        <div className="fixed inset-0 bg-[rgba(30,27,75,0.55)] backdrop-blur-sm z-[60] flex items-center justify-center p-5">
          <div className="bg-white rounded-3xl w-full max-w-[680px] max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              <div>
                <div className="text-xs font-bold text-gray-400 mb-2">어휘 (JSON 직접 수정)</div>
                <textarea className="w-full border-2 border-gray-200 rounded-xl p-3 text-xs font-mono min-h-[120px] resize-y outline-none focus:border-indigo-400"
                  value={JSON.stringify(editUnit.vocabulary, null, 2)}
                  onChange={e => { try { setEditUnit({ ...editUnit, vocabulary: JSON.parse(e.target.value) }) } catch {} }} />
              </div>
              <div>
                <div className="text-xs font-bold text-gray-400 mb-2">문법</div>
                <textarea className="w-full border-2 border-gray-200 rounded-xl p-3 text-xs font-mono min-h-[100px] resize-y outline-none focus:border-indigo-400"
                  value={JSON.stringify(editUnit.grammar, null, 2)}
                  onChange={e => { try { setEditUnit({ ...editUnit, grammar: JSON.parse(e.target.value) }) } catch {} }} />
              </div>
              <div>
                <div className="text-xs font-bold text-gray-400 mb-2">관용어</div>
                <textarea className="w-full border-2 border-gray-200 rounded-xl p-3 text-xs font-mono min-h-[80px] resize-y outline-none focus:border-indigo-400"
                  value={JSON.stringify(editUnit.idioms, null, 2)}
                  onChange={e => { try { setEditUnit({ ...editUnit, idioms: JSON.parse(e.target.value) }) } catch {} }} />
              </div>
              <div>
                <div className="text-xs font-bold text-gray-400 mb-2">쓰기 주제</div>
                <input className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-400"
...
function AssignClassModal({ textbook, availableClasses, onSave, onClose }: {
  textbook: Textbook
  ...
  const toggle = (ac: AssignedClass) => {
    const exists = selected.some(s => s.schoolId === ac.schoolId && s.semester === ac.semester && s.classId === ac.classId)
    setSelected(exists
        ...
        <div className="space-y-2 mb-6 max-h-[320px] overflow-y-auto">
          ...
          {availableClasses.map((ac, i) => (
            <label key={i} className={`flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors
              ...
              <input type="checkbox" checked={isSelected(ac)} onChange={() => toggle(ac)}
